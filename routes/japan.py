...
# 2. 일본 전용 예측 API
@japan_bp.route('/japan/predict', methods=['POST'])
def japan_predict():
    data = request.get_json() or {}
    interest_rate = float(data.get('interest', 0.25))  # 일본 기본 금리 예시

    try:
        # 일본 ID인 1번을 고정으로 전달
        result = make_prediction(target_id=1, interest_rate=interest_rate)
        ...
        # 그래프 디자인 및 가독성 세팅
        ax.set_title(f"Japan Economic Simulation (Interest Rate: {interest_rate}%)", fontsize=12, fontweight='bold')
